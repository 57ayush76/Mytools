...
def check_tools():
    ...
    for tool, install_suggestion in REQUIRED_TOOLS.items():
        ...
        # Check if tool exists in system PATH or current directory
        if result.returncode != 0:
            current_dir_check = subprocess.run(f"ls | grep {tool}", shell=True, capture_output=True, text=True)
            if current_dir_check.returncode != 0:
                print(f"[X] {tool} is missing. {install_suggestion}")
                missing_tools.append(tool)
            else:
                print(f"[✓] Found {tool} in the current directory.")
